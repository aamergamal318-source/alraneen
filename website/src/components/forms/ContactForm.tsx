  ...
  if (submitted) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-2xl p-10 text-center">
        ...
        <p className="text-green-800 font-bold text-xl">{t.contact.form.success}</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-neutral-100 p-8">
      <div className={`flex items-start gap-3 bg-gold/10 border border-gold/20 rounded-xl p-4 mb-8 ${isRTL ? 'flex-row-reverse text-right' : ''}`}>
        ...
        <p className="text-sm text-neutral-700">{t.contact.note}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            <label className={labelClass}>{t.contact.form.type} *</label>
